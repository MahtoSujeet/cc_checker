...
class Check(ParentChecker):
    ...
    def request_3(self):
        ...
        self.session.cookies.set("amzn-checkout-session", "%7B%7D")
        self.session.cookies.set("mage-cache-storage", "%7B%7D")
        self.session.cookies.set("mage-cache-storage-section-invalidation", "%7B%7D")
        self.session.cookies.set("form_key", "Gv7u6LjkINGuNnxb")
        self.session.cookies.set("PHPSESSID", "67kesr5crum6iu88tm491ddc26")
        self.session.cookies.set("wp_customerGroup", "NOT+LOGGED+IN")
        self.session.cookies.set("X-Magento-Vary", "e3cb9ab3566a693edff3edf82caa39b1ed79e8ba")
        self.session.cookies.set("recently_viewed_product", "%7B%7D")
        self.session.cookies.set("recently_viewed_product_previous", "%7B%7D")
        self.session.cookies.set("recently_compared_product", "%7B%7D")
        self.session.cookies.set("recently_compared_product_previous", "%7B%7D")
        self.session.cookies.set("product_data_storage", "%7B%7D")
        self.session.cookies.set("form_key", "Gv7u6LjkINGuNnxb")
        self.session.cookies.set("__zlcmid", "17wkcBQzB7N1OxO")
        self.session.cookies.set("amzn-checkout-session-config", "%7B%7D")
        self.session.cookies.set("language", "en_GB")
        self.session.cookies.set("ledgerCurrency", "GBP")
        self.session.cookies.set("apay-session-set", "GIn1dn19y4ofVsFu2nV%2BrQga%2ByCRj9lpDtc%2BGY11Ar%2F1teYAEhjeNAA0rEFX%2BNQ%3D")
        self.session.cookies.set("mage-cache-sessid", "true")
        self.session.cookies.set("mage-messages", "")
        self.session.cookies.set("private_content_version", "b6e7203ab7e134a999baac69ef219a1b")
        self.session.cookies.set("__kla_id", "eyIkcmVmZXJyZXIiOnsidHMiOjE2NDE2MzE1NjUsInZhbHVlIjoiIiwiZmlyc3RfcGFnZSI6Imh0dHBzOi8vd3d3LmRlcm1hY2FyZWRpcmVjdC5jby51ay8ifSwiJGxhc3RfcmVmZXJyZXIiOnsidHMiOjE2NDE2MzIwNTYsInZhbHVlIjoiIiwiZmlyc3RfcGFnZSI6Imh0dHBzOi8vd3d3LmRlcm1hY2FyZWRpcmVjdC5jby51ay8ifX0=")
        self.session.cookies.set("sociallogin_referer_store", "https%3A%2F%2Fwww.dermacaredirect.co.uk%2Fcheckout%2F%23payment")
        self.session.cookies.set("section_data_ids", "%7B%22gtm%22%3A1641632059%2C%22messages%22%3A1641632067%2C%22customer%22%3A1641631628%2C%22compare-products%22%3A1641631628%2C%22last-ordered-items%22%3A1641631628%2C%22cart%22%3A1641631886%2C%22directory-data%22%3A1641631628%2C%22captcha%22%3A1641631628%2C%22instant-purchase%22%3A1641631628%2C%22loggedAsCustomer%22%3A1641631628%2C%22persistent%22%3A1641631628%2C%22review%22%3A1641631628%2C%22wishlist%22%3A1641631628%2C%22ammessages%22%3A1641631886%2C%22recently_viewed_product%22%3A1641631628%2C%22recently_compared_product%22%3A1641631628%2C%22product_data_storage%22%3A1641631628%2C%22paypal-billing-agreement%22%3A1641631628%7D")
        res = self.session.post(url= url, headers= headers, data= postdata)
        print(res.text)
        res_json = json.loads(res.text)
        try:
